        ...
        <div className="w-full lgl:w-[737px] sml:px-8 lg:px-0 h-auto lg:h-[377px] flex flex-col gap-[32px]">
          <div className="flex flex-col lg:flex-row justify-between gap-4 lg:gap-0">
            <div className="bg-[#F5F5F5] opacity-50 w-full lg:w-[235px]">
              <input
                ...
                disabled={isLoading}
                className={`bg-[#F5F5F5] font-normal text-[16px] h-[50px] outline-none w-full py-[4px] pl-[16px] leading-[24px] ${poppins.className} ${isLoading ? 'cursor-not-allowed' : ''}`}
              />
            </div>
            <div className="bg-[#F5F5F5] opacity-50 w-full lg:w-[235px]">
              <input
            ...
            <div className="bg-[#F5F5F5] opacity-50 w-full lg:w-[235px]">
              <input
                ...
                onChange={handleChange}
                disabled={isLoading}
                className={`font-normal bg-[#F5F5F5] text-[16px] h-[50px] outline-none w-full py-[4px] pl-[16px] leading-[24px] ${poppins.className} ${isLoading ? 'cursor-not-allowed' : ''}`}
              />
            </div>
          </div>
          <div className="flex w-full gap-[16px] items-center">
            <textarea
              ...
              onChange={handleChange}
              disabled={isLoading}
              className={`font-normal resize-none bg-[#F5F5F5] text-[16px] h-[207px] outline-none w-full py-[4px] pl-[16px] leading-[24px] ${poppins.className} ${isLoading ? 'cursor-not-allowed' : ''}`}
            />
          </div>
          <div className="flex gap-[16px] justify-end items-center">
            <button
              ...
              disabled={isLoading}
              className={`relative bg-[#DB4444] text-[#ffffff] font-medium text-[16px] h-[56px] w-[215px] rounded-[4px] outline-none ${poppins.className} ${isLoading ? 'cursor-not-allowed opacity-80' : ''}`}
            >
              {isLoading ? (
