  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const orders = responses[1] as { totalAmount: number }[];

        const totalPrices: number[] = orders.map((order) => order.totalAmount);
        const totalOrderAmount: number = totalPrices.reduce((sum, price) => sum + price, 0);
    
        const ProfitPrice = totalOrderAmount + (totalOrderAmount * 0.2);
    ...
    <div className="flex items-center justify-center">
      <div className="grid grid-cols-1 smx:grid-cols-2 gap-8 md:grid-cols-3 lgl:grid-cols-4 justify-items-center">
        {[
          ...
          { icon: GiMoneyStack, count: `${data.totalOrderAmount}/=`, label: "Total  Amount" },
          { icon: GiMoneyStack, count: `${data.ProfitPrice}/=`, label: "Profit (20%)" },
        ].map((item, index) => (
