...
export default function EditOrder({ params }: EditOrderProps) {
  ...
  const [isCancelModalOpen, setIsCancelModalOpen] = useState(false); // Track cancel modal open state
  ...
  const handleCancelOrder = () => {
    setIsCancelModalOpen(true); // Open cancel confirmation modal
  };
  ...
  const confirmCancelOrder = async () => {
    if (data && data._id) {
      try {
        ...
        router.push("/orders"); // Redirect to orders page after deletion
      } catch (error) {
